...
for p in range(1, 5):
	...
	model = SVC(C=1.0, kernel='poly', degree=p, gamma=1)
	model.fit(trainData, trainLabels)
	predictions = model.predict(testData)
	print(classification_report(testLabels, predictions, target_names=le.classes_))
	
for l in range(1, 11):	
	print "Sluoksniu skaicius = %d" % (l)
	model = MLPClassifier(hidden_layer_sizes=(l, ) ,solver='sgd', learning_rate_init=0.01, max_iter=500)
	model.fit(trainData, trainLabels)
	predictions = model.predict(testData)
	print(classification_report(testLabels, predictions, target_names=le.classes_))